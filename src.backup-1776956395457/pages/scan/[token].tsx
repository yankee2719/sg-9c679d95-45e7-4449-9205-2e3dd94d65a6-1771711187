...
import { cacheQrTokenForOffline, validateCachedQrToken } from "@/lib/offlineQrCache";
...
export default function ScanPage() {
    ...
    const tryOfflineAccess = (tokenValue: string) => {
        setIsOffline(true);
        const offlineResult = validateCachedQrToken(tokenValue);
    ...
    const handleScan = async (tokenValue: string) => {
        ...
        try {
            ...
            if (res.ok) {
                ...
                cacheQrTokenForOffline(tokenValue, {
                    equipment_id,
                    allowed_views,
                ...
                {state === "auth_required" && (
                    <div className="space-y-4">
                        ...
                        <div className="flex justify-center gap-3">
                            ...
                            <Button onClick={() => void router.push("/login")}>{t("auth.signIn") || "Accedi"}</Button>
                        </div>
                    </div>
                )}
