...
const I18N: Record<Language, Record<string, string>> = {
  it: {
    title: "Documenti",
    subtitle: "Gestione documenti dell'organizzazione.",
    manufacturerView: "Vista costruttore",
    customerView: "Vista cliente",
    total: "Documenti totali",
    categories: "Categorie",
    withMachine: "Con macchina",
    search: "Cerca documento...",
    all: "Tutti",
    loading: "Caricamento...",
    noResults: "Nessun documento trovato",
    noMachine: "Documento generale",
    versions: "Versioni",
  },
  en: {
    title: "Documents",
    subtitle: "Organization document management.",
    ...
    noMachine: "General document",
    versions: "Versions",
  },
  fr: {
    title: "Documents",
    subtitle: "Gestion des documents de l'organisation.",
    manufacturerView: "Vue constructeur",
    customerView: "Vue client",
    total: "Documents totaux",
    categories: "Catégories",
    withMachine: "Avec machine",
    search: "Rechercher un document...",
    all: "Tous",
    loading: "Chargement...",
    noResults: "Aucun document trouvé",
    noMachine: "Document général",
    versions: "Versions",
  },
  es: {
    title: "Documentos",
    subtitle: "Gestión documental de la organización.",
    manufacturerView: "Vista fabricante",
    customerView: "Vista cliente",
    total: "Documentos totales",
    categories: "Categorías",
    withMachine: "Con máquina",
    search: "Buscar documento...",
    all: "Todos",
    loading: "Cargando...",
    noResults: "No se encontraron documentos",
    noMachine: "Documento general",
    versions: "Versiones",
  },
};
        ...
        <div className="container mx-auto max-w-7xl space-y-6 px-4 py-8">
          <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            ...
            <div className="flex items-center gap-2 text-xs text-muted-foreground">
              ...
              <span>{orgType === "manufacturer" ? L.manufacturerView : L.customerView}</span>
            </div>
          </div>

          <div className="grid gap-4 md:grid-cols-3">
            <Card>
              <CardHeader className="pb-2"><CardTitle className="text-sm font-medium">{L.total}</CardTitle></CardHeader>
              <CardContent><div className="text-3xl font-semibold">{documents.length}</div></CardContent>
            </Card>
            <Card>
              <CardHeader className="pb-2"><CardTitle className="text-sm font-medium">{L.categories}</CardTitle></CardHeader>
              <CardContent><div className="text-3xl font-semibold">{categories.length}</div></CardContent>
            </Card>
            <Card>
              <CardHeader className="pb-2"><CardTitle className="text-sm font-medium">{L.withMachine}</CardTitle></CardHeader>
          ...
          <Card>
            <CardContent className="grid gap-4 p-5 md:grid-cols-[1fr_220px]">
              <div className="relative">
                ...
                <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder={L.search} className="h-11 w-full rounded-2xl border border-border bg-background pl-11 pr-4 text-sm text-foreground outline-none placeholder:text-muted-foreground" />
              </div>
              <div className="relative">
          ...
          <div className="space-y-4">
            {loading ? (
              ...
              <Card><CardContent className="p-6 text-sm text-muted-foreground">{L.noResults}</CardContent></Card>
            ) : documents.map((doc) => (
                  ...
                  <CardContent className="flex items-start justify-between gap-4 p-5">
                    <div className="min-w-0 space-y-2">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <div className="truncate text-lg font-semibold text-foreground">{doc.title || L.title}</div>
                          <div className="text-sm text-muted-foreground">{doc.machine_label || L.noMachine}</div>
                        </div>
                      </div>
                      ...
                      <div className="flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
                        ...
                        <span>{L.versions}: {doc.version_count || 1}</span>
