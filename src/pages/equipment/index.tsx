...
export default function EquipmentPage() {
        ...
        try {
            ...
            if (type === "customer") {
                ...
                allMachines = (machineData ?? []).map((m: any) => ({
                    ...
                    _isOwnedByActiveOrg: (m.organization_id ?? null) === orgId,
                    _isAssignedToCustomer: assignedIds.has(m.id),
                }));

                const canGroupCustomer =
                    userCtx.role === "admin" || userCtx.role === "supervisor";
            ...
            if (type === "manufacturer") {
                const { data: ownMachineRows, error: ownMachinesErr } = await supabase
                    .from("machines")
                    .select(
                        "id, name, internal_code, category, serial_number, model, brand, position, lifecycle_state, qr_code_token, plant_id, photo_url, organization_id, is_archived"
                    )
                ...
                if (ownMachinesErr) throw ownMachinesErr;
                ...
                let assignedMachineRows: any[] = [];

                if (assignedMachineIds.length > 0 && tab === "active") {
                    ...
                    assignedMachineRows = assignedRows ?? [];
                }

                const byId = new Map < string, Machine> ();

                for (const m of ownMachineRows ?? []) {
                    byId.set(m.id, {
                        ...(m as Machine),
                        _isArchived: !!m.is_archived,
                        _isOwnedByActiveOrg: true,
                        _customerOrgId: null,
                        _isAssignedToCustomer: false,
                    });
                }

                const assignmentByMachine = new Map < string, string> ();
                for (const a of assignmentRows) {
                    if (a.machine_id && a.customer_org_id) {
                        assignmentByMachine.set(a.machine_id, a.customer_org_id);
                    }
                }

                for (const m of assignedMachineRows) {
                    const existing = byId.get(m.id);

                    if (existing) {
                        byId.set(m.id, {
                            ...existing,
                            _customerOrgId: assignmentByMachine.get(m.id) ?? existing._customerOrgId ?? null,
                        ...
                        byId.set(m.id, {
                            ...
                            _isArchived: !!m.is_archived,
                            _isOwnedByActiveOrg: (m.organization_id ?? null) === orgId,
                            _customerOrgId: assignmentByMachine.get(m.id) ?? null,
                            _isAssignedToCustomer: true,
                        });
                    }
                }

                allMachines = Array.from(byId.values()).sort((a, b) =>
    ...
    const archiveMachine = async (e: React.MouseEvent, m: Machine) => {
        ...
        if (!m._isOwnedByActiveOrg) {
            toast({
                title: "Azione non consentita",
                description: "Puoi archiviare solo macchine possedute dalla tua organizzazione.",
                variant: "destructive",
            });
            return;
        }
    ...
    const renderMachineCard = (item: Machine) => {
        ...
        const isOwnedByActiveOrg = !!item._isOwnedByActiveOrg;
            ...
            >
                <CardContent className="p-4">
                                ...
                                <>
                                    ...
                                    {canArchiveMachine && (
                                        <button
                                            ...
                                            disabled={archiving === item.id || isAssigned || !isOwnedByActiveOrg}
                                            title={
                                                !isOwnedByActiveOrg
                                                    ? "Puoi archiviare solo macchine possedute dalla tua organizzazione"
                                                    : isAssigned
                    ...
                    <div className="mt-3 flex flex-wrap items-center gap-2">
                        ...
                        {isManufacturer && item._isOwnedByActiveOrg && (
                            <Badge className="rounded-full px-2.5 py-0.5 text-xs font-medium bg-blue-50 dark:bg-blue-500/10 text-blue-600 dark:text-blue-400 border-blue-200 dark:border-blue-500/30">
                                Propria
                            </Badge>
                        )}
