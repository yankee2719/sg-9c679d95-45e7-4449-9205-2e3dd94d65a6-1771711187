...
export default function EquipmentPage() {
  ...
  const EquipmentCard = ({ item }: { item: Equipment }) => {
    ...
    return (
      ...
      >
        <div className="h-32 bg-slate-700/50 relative overflow-hidden">
        ...
        <CardContent className="p-4">
          <div className="flex items-start justify-between mb-1">
            ...
            <ChevronRight className="w-4 h-4 text-slate-500 group-hover:text-blue-400 transition-colors flex-shrink-0 ml-1" />
          </div>
          <p className="text-xs text-slate-500 mb-2 font-mono">{item.equipment_code}</p>
          {item.location && (
          ...
          <div className="flex items-center justify-between">
            <span className="text-xs text-slate-500">{item.category || t("equipment.generic")}</span>
    ...
    return (
      <div className="space-y-4">
        {plants.map((plant) => {
          ...
          return (
            <div key={plant.id} className="rounded-2xl border border-blue-500/30 bg-slate-800/30 overflow-hidden">
              {/* Plant header */}
              <button
              ...
              >
                ...
                <span className="text-white font-bold text-lg">{plant.name}</span>
              ...
              {isExpanded && (
                <div className="px-4 pb-4 space-y-3">
                  ...
                  {plantDepts.map((dept) => {
                    ...
                    return (
                      <div key={dept.id} className="rounded-xl border border-amber-500/20 bg-slate-800/40 overflow-hidden ml-4">
                        {/* Department header */}
                        <button
                        ...
                        {isDeptExpanded && deptEquipment.length === 0 && (
                          <p className="px-4 pb-3 text-sm text-slate-500 ml-8">Nessuna attrezzatura in questo reparto</p>
                        )}
                      </div>
                    );
                  })}
        ...
        {unassigned.length > 0 && (
          <div className="rounded-2xl border border-slate-600/30 bg-slate-800/30 overflow-hidden">
            <div className="flex items-center gap-3 px-5 py-4">
              <Wrench className="w-5 h-5 text-slate-400" />
              <span className="text-slate-300 font-bold text-lg">Non assegnate</span>
              <Badge className="bg-slate-500/20 text-slate-400 border-slate-500/30 ml-2">
                {unassigned.length}
              </Badge>
            </div>
