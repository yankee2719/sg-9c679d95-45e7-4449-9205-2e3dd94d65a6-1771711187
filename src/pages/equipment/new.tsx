...
export default function NewEquipmentPage() {
  ...
  useEffect(() => {
    ...
    const init = async () => {
      try {
        ...
        // ✅ IMPORTANTISSIMO: ricavo SEMPRE il tipo org dal DB (non fidarti di ctx.orgType)
        ...
        // Caricamenti dipendono dal tipo
        if (resolvedType === "manufacturer") {
          // manufacturer -> carico lista clienti
          const { data, error } = await supabase
            .from("organizations")
            .select("id,name")
          ...
          // customer -> carico stabilimenti dell'org
          const { data, error } = await supabase
            .from("plants")
            .select("id,name,code")
  ...
  useEffect(() => {
    // Solo customer: carico linee quando scelgo plant
    if (!orgId) return;
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Manufacturer -> assegna a customer tramite machine_assignments
      if (isManufacturer && selectedCustomerId && machine?.id) {
        ...
        if (assignError) {
          // non blocco la creazione macchina, ma mostro warning
