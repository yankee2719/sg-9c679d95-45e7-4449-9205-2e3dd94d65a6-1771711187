            ...
            <MainLayout>
                <div className="flex items-center justify-center min-h-[60vh]">
                    <div className="text-center">
                        <p className="text-muted-foreground mb-4">{t("equipment.invalidId") || "ID equipaggiamento non valido"}</p>
                        <Link href="/equipment" className="text-primary hover:underline">{t("equipment.backToList") || "Torna alla lista"}</Link>
                    </div>
                </div>
            </MainLayout>
        );
            ...
            <div className="space-y-6 max-w-5xl mx-auto">
                <div>
                    <Link href={`/equipment/${id}`} className="inline-flex items-center gap-2 text-sm text-muted-foreground hover:text-foreground mb-4 transition-colors">
                        ...
                        {t("equipment.backToDetail") || "Torna all'equipaggiamento"}
                    </Link>
                    <div className="flex items-start justify-between">
                        <div>
                            <h1 className="text-2xl font-bold text-foreground flex items-center gap-3">
                                <FileText className="w-7 h-7 text-primary" />
                                {t("equipment.documentManagement") || "Gestione Documenti"}
                            </h1>
                ...
                <div className="bg-blue-500/10 border border-blue-500/20 rounded-lg p-6">
                    <h3 className="text-sm font-semibold text-blue-400 mb-2">{t("equipment.docBestPractices") || "Best Practices Documentazione"}</h3>
