...
export default function UsersIndexPage() {
    ...
    const stats = useMemo(
        () => ({
            ...
            admins: rows.filter((row) => ["owner", "admin", "supervisor"].includes(String(row.role || "").toLowerCase())).length,
            viewers: rows.filter((row) => String(row.role || "").toLowerCase() === "viewer").length,
        }),
        [rows]
    );
                                        ...
                                        >
                                            <div className="space-y-2">
                                                <div className="flex flex-wrap items-center gap-3">
                                                    ...
                                                    <UserRoleBadge role={row.role || "viewer"} />
                                                </div>
