...
async function listUsers(req: AuthenticatedRequest, res: NextApiResponse) {
    ...
    let profiles: any[] = [];
    if (userIds.length > 0) {
        const { data: profileRows, error: profileError } = await serviceSupabase.from("profiles").select("id, email, display_name, first_name, last_name, avatar_url").in("id", userIds);
...
async function createUser(req: AuthenticatedRequest, res: NextApiResponse) {
    ...
    if (!req.user.organizationId) return res.status(400).json({ error: "No active organization context" });
    if (!email || !password || !isValidOrganizationUserRole(role)) return res.status(400).json({ error: "Missing or invalid required fields" });
    ...
    try {
        ...
        return res.status(201).json({ data: { ok: true, user_id: created.userId, membership_id: created.membershipId, email: created.email } });
    } catch (error) {
        console.error("/api/users POST error:", error);
        if (error instanceof UserProvisioningError) return res.status(error.statusCode).json({ error: error.message });
...
export default withAuth(ALL_APP_ROLES, handler, { requireAal2: false });
