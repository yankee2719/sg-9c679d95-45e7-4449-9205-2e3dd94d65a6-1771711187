...
export default function MachineEditorPage({ mode, machineId = null }: MachineEditorPageProps) {
    ...
    useEffect(() => {
        ...
        const load = async () => {
            ...
            try {
                setLoading(true);
                if (orgType === "customer" || orgType === "enterprise") {
    ...
    const handleSave = async () => {
        ...
        try {
            const isEndUser = orgType === "customer" || orgType === "enterprise";
            const payload = { name: form.name.trim(), internal_code: form.internal_code.trim() || null, serial_number: form.serial_number.trim() || null, model: form.model.trim() || null, brand: form.brand.trim() || null, lifecycle_state: form.lifecycle_state || "active", notes: form.notes.trim() || null, plant_id: isEndUser ? form.plant_id || null : null, production_line_id: isEndUser ? form.production_line_id || null : null };
            if (mode === "create") {
