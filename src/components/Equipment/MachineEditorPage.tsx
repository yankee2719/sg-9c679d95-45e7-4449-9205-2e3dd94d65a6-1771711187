...
export default function MachineEditorPage({ mode, machineId = null }: MachineEditorPageProps) {
    ...
    const pageTitle = mode === "create" ? (t("equipment.new") || "Nuova macchina") : (t("equipment.editMachine") || "Modifica macchina");
    const pageSubtitle = mode === "create" ? (t("equipment.createSubtitle") || "Inserisci i dati principali della macchina nel contesto organizzativo attivo.") : (t("equipment.editSubtitle") || "Aggiorna i dati principali della macchina nel contesto organizzativo attivo.");
    ...
    const handleSave = async () => {
        ...
        if (!form.name.trim()) {
            toast({ title: t("common.error") || "Errore", description: t("equipment.requiredName") || "Il nome macchina è obbligatorio.", variant: "destructive" });
            return;
        }
        setSaving(true);
        try {
            ...
            if (mode === "create") {
                ...
                toast({ title: t("equipment.created") || "Macchina creata", description: form.name.trim() });
                void router.push(`/equipment/${data.id}`);
            } else if (machineId) {
                await updateMachine(machineId, payload);
                toast({ title: t("equipment.updated") || "Macchina aggiornata", description: form.name.trim() });
                void router.push(`/equipment/${machineId}`);
            }
        } catch (error: any) {
            console.error("Machine save error:", error);
            toast({ title: t("common.error") || "Errore", description: error?.message || (t("equipment.genericSaveError") || "Errore durante il salvataggio della macchina"), variant: "destructive" });
        } finally {
            setSaving(false);
        }
    };
    ...
    if (!orgId || !orgType || !canEdit) {
        return <OrgContextGuard><MainLayout userRole={userRole}><SEO title={`${pageTitle} - MACHINA`} /><div className="mx-auto max-w-5xl px-4 py-8"><Card className="rounded-2xl"><CardContent className="py-10 text-center text-muted-foreground">{t("equipment.forbidden") || "Questa pagina non è disponibile nel contesto attivo."}</CardContent></Card></div></MainLayout></OrgContextGuard>;
    }
        ...
        <OrgContextGuard>
            <MainLayout userRole={userRole}>
                ...
                <div className="mx-auto max-w-5xl space-y-6 px-4 py-8">
                    ...
                    <Card className="rounded-2xl"><CardHeader><CardTitle className="flex items-center gap-2"><Settings2 className="h-5 w-5" />{t("equipment.mainData") || "Dati principali"}</CardTitle></CardHeader><CardContent className="grid gap-4 md:grid-cols-2"><div className="space-y-2 md:col-span-2"><Label>{t("equipment.name") || "Nome macchina *"}</Label><Input value={form.name} onChange={(e) => handleField("name", e.target.value)} placeholder={t("equipment.placeholderName") || "Es. HMS 140"} /></div><div className="space-y-2"><Label>{t("equipment.internalCode") || "Codice interno"}</Label><Input value={form.internal_code} onChange={(e) => handleField("internal_code", e.target.value)} /></div><div className="space-y-2"><Label>{t("machines.serialNumber") || "Matricola"}</Label><Input value={form.serial_number} onChange={(e) => handleField("serial_number", e.target.value)} /></div><div className="space-y-2"><Label>{t("machines.manufacturer") || "Marca"}</Label><Input value={form.brand} onChange={(e) => handleField("brand", e.target.value)} /></div><div className="space-y-2"><Label>{t("machines.model") || "Modello"}</Label><Input value={form.model} onChange={(e) => handleField("model", e.target.value)} /></div><div className="space-y-2 md:col-span-2"><Label>{t("machines.status") || "Stato macchina"}</Label><select value={form.lifecycle_state} onChange={(e) => handleField("lifecycle_state", e.target.value)} className="h-10 w-full rounded-md border border-input bg-background px-3 text-sm"><option value="active">{t("equipment.lifecycle.active") || "Attiva"}</option><option value="inactive">{t("equipment.lifecycle.inactive") || "Inattiva"}</option><option value="under_maintenance">{t("equipment.lifecycle.maintenance") || "In manutenzione"}</option><option value="commissioning">{t("equipment.lifecycle.commissioning") || "In commissioning"}</option><option value="decommissioned">{t("equipment.lifecycle.decommissioned") || "Dismessa"}</option></select></div><div className="space-y-2 md:col-span-2"><Label>{t("equipment.notes") || "Note"}</Label><Textarea value={form.notes} onChange={(e) => handleField("notes", e.target.value)} rows={4} /></div></CardContent></Card>
                    <Card className="rounded-2xl"><CardHeader><CardTitle className="flex items-center gap-2">{orgType === "manufacturer" ? <Factory className="h-5 w-5" /> : <Building2 className="h-5 w-5" />}{t("equipment.organizationContext") || "Contesto organizzativo"}</CardTitle></CardHeader><CardContent className="space-y-4"><div className="rounded-2xl border border-border bg-muted/30 p-4"><div className="text-sm text-muted-foreground">{orgType === "manufacturer" ? (t("org.manufacturer") || "Costruttore") : (t("org.customer") || "Cliente finale")}</div><div className="mt-1 font-semibold text-foreground">{t("equipment.owner") || "Organizzazione attiva"}: {orgName}</div></div>{orgType === "customer" && <div className="grid gap-4 md:grid-cols-2"><div className="space-y-2"><Label className="flex items-center gap-2"><MapPin className="h-4 w-4" />{t("plants.fallbackPlant") || "Stabilimento"}</Label><select value={form.plant_id} onChange={(e) => handlePlantChange(e.target.value)} className="h-10 w-full rounded-md border border-input bg-background px-3 text-sm"><option value="">{t("common.none") || "Nessuno"}</option>{plants.map((plant) => <option key={plant.id} value={plant.id}>{plant.name || plant.code || plant.id}</option>)}</select></div><div className="space-y-2"><Label className="flex items-center gap-2"><GitBranch className="h-4 w-4" />{t("plants.line") || "Linea"}</Label><select value={form.production_line_id} onChange={(e) => handleField("production_line_id", e.target.value)} className="h-10 w-full rounded-md border border-input bg-background px-3 text-sm"><option value="">{t("common.none") || "Nessuna"}</option>{lines.map((line) => <option key={line.id} value={line.id}>{line.name || line.code || line.id}</option>)}</select></div>{plants.length === 0 && <div className="rounded-2xl border border-dashed border-border bg-muted/30 p-4 text-sm text-muted-foreground md:col-span-2">{t("equipment.noPlantsAvailable") || "Non risultano stabilimenti nel contesto attivo."} <Link href="/plants" className="font-medium text-orange-500 hover:underline">{t("equipment.createPlantsCta") || "Apri stabilimenti"}</Link></div>}</div>}</CardContent></Card>
